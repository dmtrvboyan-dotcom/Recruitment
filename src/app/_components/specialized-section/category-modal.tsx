...
export const CategoryModal = memo(function CategoryModal({ category }: { category: TechCategory }) {
  ...
  return (
    <div className="p-4 sm:p-6 max-h-[85vh] overflow-y-auto">
      ...
      <div className="w-11 h-11 sm:w-14 sm:h-14 rounded-2xl bg-brand-white flex items-center justify-center mb-4 sm:mb-5">
          ...
          <ul className="space-y-2 sm:space-y-2.5">
            {category.howWeSource.map((item) => (
              <li key={item} className="flex items-start gap-2.5 text-xs sm:text-sm text-brand-navy/65 leading-snug">
                <span className="mt-1.25 w-1.5 h-1.5 rounded-full bg-brand-navy/25 shrink-0" />
                {item}
              </li>
            ))}
          </ul>
      ...
      <div className="grid grid-cols-2 gap-3 sm:gap-4 bg-gray-50 rounded-2xl p-4 sm:p-5">
        <div className="flex items-center gap-2.5 sm:gap-3">
          <div className="w-9 h-9 sm:w-10 sm:h-10 rounded-xl bg--brand-white border border-gray-100 flex items-center justify-center shrink-0">
