...
export function JobsSection() {
  ...
  return (
    ...
    >
      ...
      <div
        aria-hidden
        className="hidden lg:block absolute top-1/2 right-0 -translate-y-1/2 text-[clamp(10rem,18vw,16rem)] font-bold uppercase leading-[0.85] tracking-tighter text-brand-white/2.5select-none pointer-events-none whitespace-nowrap text-brand-navy/1"
      >
        Open Roles
      </div>
...
Job <span className="text-brand-coral">Listings</span>
        ...
        <div className="relative group max-w-xl mx-auto mb-12 lg:mb-16">
          <Search className="absolute left-6 top-1/2 -translate-y-1/2 text-brand-navy/40 group-focus-within:text-brand-coral w-5 h-5 transition-colors duration-200" />
          <input
            type="text"
            placeholder="Search by role or tech stack..."
            value={searchQuery}
            onChange={(e) => setSearchQuery(e.target.value)}
            className="w-full bg-brand-navy/5 border border-brand-white/10 pl-14 pr-14 py-5 rounded-full text-brand-navy placeholder:text-brand-navy/50 focus:outline-none focus:border-brand-coral/50 focus:bg-brand-navy/[0.07] transition-colors duration-200 text-sm sm:text-base"
          />
          {searchQuery && (
            <button
              onClick={() => setSearchQuery("")}
              className="absolute right-6 top-1/2 -translate-y-1/2 text-brand-navy/40 hover:text-brand-coral transition-colors duration-200 cursor-pointer"
            >
        ...
        <div className="flex flex-col lg:flex-row gap-8 lg:gap-10">
          ...
          <div className="flex-1 min-w-0">
            <div className="mb-8 lg:mb-10 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 pb-6 border-b border-brand-white/10">
              <div className="flex items-center gap-3">
                ...
                <div className="flex items-baseline gap-2.5">
                  <span className="text-2xl lg:text-3xl font-bold text-brand-navy tabular-nums tracking-tight leading-none">
                    {filteredJobs.length}
                  </span>
            ...
            {filteredJobs.length === 0 && (
              <div className="text-center py-20 lg:py-32 rounded-3xl bg-brand-navy/2 border border-dashed border-brand-navy/10">
                <SlidersHorizontal
                  className="w-12 h-12 text-brand-navy/20 mx-auto mb-5"
                  strokeWidth={1.5}
                />
                <h3 className="text-base font-bold uppercase tracking-tight text-brand-navy mb-2">
                  No briefs match those filters
                </h3>
                <p className="text-sm text-brand-navy/45 mb-6">
