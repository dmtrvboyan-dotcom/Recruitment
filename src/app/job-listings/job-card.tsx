...
export const JobCard = memo(function JobCard({
  ...
  return (
    <button
      onClick={onSelect}
      className="group relative w-full text-left rounded-3xl bg-brand-navy/2 border border-brand-navy/10 hover:border-brand-coral/50 hover:bg-brand-white/4] transition-colors duration-300 p-6 lg:p-7 flex flex-col gap-5 overflow-hidden cursor-pointer"
    >
      {/* Top row: number + status pill */}
      <div className="flex items-start justify-between">
        <div className="flex items-center gap-2.5">
          <span className="text-3xl lg:text-4xl font-bold leading-[0.85] tracking-[-0.04em] text-brand-navy/10 group-hover:text-brand-coral/50 transition-colors duration-300 tabular-nums">
            {number}
          </span>
      ...
      {/* Title */}
      <h3 className="font-bold uppercase tracking-[-0.015em] leading-[1.05] text-brand-navy text-xl lg:text-[1.5rem] line-clamp-2">
        {job.title}
      </h3>

      {/* Meta row */}
      <div className="flex flex-wrap items-center gap-x-4 gap-y-1.5 text-[12px] text-brand-navy/55">
