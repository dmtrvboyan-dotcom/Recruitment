  ...
  return (
    <div className="flex flex-col gap-1.5">
      ...
      <p className="text-[10px] text-gray-400">
        URL copied. Paste as <code className="bg-gray-100 px-1 rounded">image: &quot;URL&quot;</code> or <code className="bg-gray-100 px-1 rounded">![alt](URL)</code>.
      </p>
    ...
    <div className="bg-white border border-gray-200 rounded-2xl p-5 flex flex-col gap-4">
      ...
      <div>
        <label className="text-xs font-medium text-gray-600 mb-1 flex items-center gap-2">
          Slug (URL)
          {slugLocked && <span className="text-[10px] bg-gray-100 text-gray-500 px-1.5 py-0.5 rounded-full">🔒 Locked</span>}
        </label>
        <input
          type="text"
          value={form.slug}
      ...
      <div className="bg-amber-50 border border-amber-200 rounded-xl p-3 flex items-start gap-3">
        ...
        />
        <label htmlFor="featured-toggle" className="cursor-pointer flex-1">
          <p className="text-sm font-medium text-amber-900">⭐ Featured post for this tab</p>
          <p className="text-[11px] text-amber-700 mt-0.5">
            Shows as a large card at the top of the blog. Only one post per tab can be featured — marking this will automatically unfeature the previous one.
          </p>
        </label>
      </div>
    </div>
...
export default function AdminPage() {
  ...
  async function saveDraftMetadata(updated: MetadataState) {
    ...
    if (res.ok) {
      ...
      // Refetch drafts since auto-unfeature may have touched others
      await fetchDrafts();
  ...
  async function saveEdit(slug: string) {
    ...
    if (res.ok) {
      ...
      setDrafts((d) => d.map((draft) => draft.slug === slug ? { ...draft, content: editContent } : draft));
      setIsEditing(false);
  ...
  async function savePublishedMetadata(updated: MetadataState) {
    ...
    if (res.ok) {
      ...
      // Refetch to get fresh SHA after our edit (and potential sibling edits)
  ...
  async function savePublishedEdit() {
    ...
    const res = await fetch("/api/admin/update-post", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ slug: selectedPost.slug, content: editPostContent, sha: postSha }),
    });
    if (res.ok) {
      ...
      <header className="bg-white border-b border-gray-200 px-6 py-4 flex items-center justify-between">
        <div>
          ...
          <div className="flex gap-4 mt-2">
            <button onClick={() => { setTab("drafts"); setPreview(null); }} className={`text-sm font-medium pb-0.5 border-b-2 transition-colors ${tab === "drafts" ? "border-gray-900 text-gray-900" : "border-transparent text-gray-400 hover:text-gray-600"}`}>
              Drafts{drafts.length > 0 && <span className="ml-1 text-xs bg-gray-900 text-white px-1.5 py-0.5 rounded-full">{drafts.length}</span>}
            </button>
            <button onClick={() => { setTab("published"); setPreview(null); }} className={`text-sm font-medium pb-0.5 border-b-2 transition-colors ${tab === "published" ? "border-gray-900 text-gray-900" : "border-transparent text-gray-400 hover:text-gray-600"}`}>
              Published
            </button>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3">
          <button onClick={triggerGeneration} className="text-sm bg-gray-900 text-white px-4 py-2 rounded-lg hover:bg-gray-700 transition">+ Generate Now</button>
          <button onClick={handleLogout} className="text-sm text-gray-400 hover:text-red-500 transition px-3 py-2 rounded-lg hover:bg-red-50">Log out</button>
        </div>
      </header>
      ...
      <div className="flex h-[calc(100vh-85px)]">
        <aside className="w-96 shrink-0 border-r border-gray-200 bg-white overflow-y-auto">
          {loading ? (
            ...
            drafts.length === 0 ? (
              <div className="p-8 text-center"><p className="text-gray-400 text-sm">No drafts yet.</p></div>
            ) : (
              <ul className="divide-y divide-gray-100">
                {drafts.map((draft) => {
                  const isActive = preview?.slug === draft.slug;
                  const state = actionState[draft.slug];
                  const isFeatured = /^featured:\s*true/m.test(draft.content);
                  return (
                    <li key={draft.slug} onClick={() => { setPreview(draft); setIsEditing(false); }} className={`p-4 cursor-pointer transition ${isActive ? "bg-gray-50" : "hover:bg-gray-50"}`}>
                      <div className="flex gap-2 mb-2 flex-wrap">
                        <span className={`text-xs font-medium px-2 py-0.5 rounded-full ${THEME_COLORS[draft.themeId] ?? "bg-gray-100 text-gray-600"}`}>{draft.themeLabel}</span>
                        <span className="text-xs font-medium px-2 py-0.5 rounded-full bg-gray-100 text-gray-500">{draft.category}</span>
                        {isFeatured && <span className="text-xs font-medium px-2 py-0.5 rounded-full bg-amber-100 text-amber-700">⭐ Featured</span>}
                      </div>
                      ...
                      <div className="flex gap-2 mt-3" onClick={(e) => e.stopPropagation()}>
                        <button onClick={() => publish(draft.slug)} disabled={!!state} className="flex-1 text-xs bg-emerald-600 text-white py-1.5 rounded-lg hover:bg-emerald-700 transition disabled:opacity-50">
                          {state === "publishing" ? "Publishing…" : "✓ Publish"}
                        </button>
                        <button onClick={() => deleteDraft(draft.slug)} disabled={!!state} className="text-xs bg-gray-100 text-gray-600 px-3 py-1.5 rounded-lg hover:bg-red-50 hover:text-red-600 transition disabled:opacity-50">
                          {state === "deleting" ? "…" : "✕"}
                        </button>
                      </div>
                    </li>
            ...
            published.length === 0 ? (
              ...
              <ul className="divide-y divide-gray-100">
                {published.map((post) => {
                  ...
                  return (
                    <li key={post.slug} onClick={() => selectPublishedPost(post)} className={`p-4 cursor-pointer transition flex items-center justify-between gap-3 ${isSelected ? "bg-gray-50" : "hover:bg-gray-50"}`}>
                      ...
                      <button onClick={(e) => { e.stopPropagation(); deletePublished(post.slug); }} disabled={!!state} className="shrink-0 text-xs bg-red-50 text-red-600 px-3 py-1.5 rounded-lg hover:bg-red-100 transition disabled:opacity-50">
            ...
            <div className="max-w-3xl mx-auto p-8 space-y-5">
              <MetadataEditor
                initial={{
                  ...
                  slug: preview.slug,
                  featured: /^featured:\s*true/m.test(preview.content),
                }}
                slugLocked={false}
              ...
              <div className="flex items-center justify-between bg-white border border-gray-200 rounded-2xl p-4">
                ...
                <div className="flex gap-2">
                  {isEditing ? (
                    <>
                      <button onClick={() => saveEdit(preview.slug)} disabled={saving} className="text-sm bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700 transition disabled:opacity-50">
                        {saving ? "Saving…" : "Save content"}
                      </button>
                      <button onClick={() => { setIsEditing(false); setEditContent(""); }} disabled={saving} className="text-sm bg-gray-100 text-gray-600 px-4 py-2 rounded-lg hover:bg-gray-200 transition">Cancel</button>
                    </>
                  ) : (
                    <>
                      <button onClick={() => { setIsEditing(true); setEditContent(preview.content); }} className="text-sm bg-gray-100 text-gray-700 px-4 py-2 rounded-lg hover:bg-gray-200 transition">✎ Edit content</button>
                      <button onClick={() => publish(preview.slug)} disabled={!!actionState[preview.slug]} className="text-sm bg-emerald-600 text-white px-4 py-2 rounded-lg hover:bg-emerald-700 transition disabled:opacity-50">
                        {actionState[preview.slug] === "publishing" ? "Publishing…" : "✓ Publish"}
                      </button>
                      <button onClick={() => deleteDraft(preview.slug)} className="text-sm bg-gray-100 text-gray-600 px-4 py-2 rounded-lg hover:bg-red-50 hover:text-red-600 transition">Delete</button>
                    </>
                  )}
                </div>
              </div>
              ...
              {isEditing ? (
                ...
                <pre className="bg-gray-900 text-gray-100 text-xs p-6 rounded-2xl overflow-x-auto whitespace-pre-wrap leading-relaxed">{preview.content}</pre>
              )}
            </div>
            ...
            <div className="max-w-3xl mx-auto p-8 space-y-5">
              ...
              <div className="flex items-center justify-between bg-white border border-gray-200 rounded-2xl p-4">
                ...
                <div className="flex gap-2">
                  {isEditingPost ? (
                    <>
                      <button onClick={savePublishedEdit} disabled={savingPost} className="text-sm bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700 transition disabled:opacity-50">
                        {savingPost ? "Saving…" : "Save content"}
                      </button>
                      <button onClick={() => { setIsEditingPost(false); setEditPostContent(""); }} disabled={savingPost} className="text-sm bg-gray-100 text-gray-600 px-4 py-2 rounded-lg hover:bg-gray-200 transition">Cancel</button>
                    </>
                  ) : (
                    <>
                      <button onClick={() => { setIsEditingPost(true); setEditPostContent(postContent); }} disabled={loadingPost} className="text-sm bg-gray-100 text-gray-700 px-4 py-2 rounded-lg hover:bg-gray-200 transition disabled:opacity-50">✎ Edit content</button>
                      <button onClick={() => deletePublished(selectedPost.slug)} disabled={!!actionState[selectedPost.slug]} className="text-sm bg-red-50 text-red-600 px-4 py-2 rounded-lg hover:bg-red-100 transition disabled:opacity-50">
              ...
              {loadingPost ? (
                ...
                <pre className="bg-gray-900 text-gray-100 text-xs p-6 rounded-2xl overflow-x-auto whitespace-pre-wrap leading-relaxed">{postContent}</pre>
              )}
            </div>
          ) : (
